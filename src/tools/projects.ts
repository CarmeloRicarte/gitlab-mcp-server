...
export function registerProjectTools(server: McpServer, client: GitLabClient) {
	server.registerTool(
		"list_projects",
		{
			...
			inputSchema: {
				...
				per_page: z.number().optional().default(20).describe("Number of results per page"),
				group: z
					.string()
					.optional()
					.describe(
						"Group ID or path to filter projects by group (e.g., 'my-org' or 'my-org/subgroup')",
					),
			},
		},
		async ({ search, per_page, group }) => {
			const params = new URLSearchParams({ per_page: String(per_page || 20) });
			if (search) params.set("search", search);

			const endpoint = group
				? `/groups/${encodeURIComponent(group)}/projects?${params}`
				: `/projects?${params}`;
			const projects = await client.get<GitLabProject[]>(endpoint);
	...
	server.registerTool(
		"get_project",
		{
			...
			inputSchema: {
				project: z.string().describe("Project ID or path (e.g., 'group/project')"),
			},
		},
		async ({ project }) => {
			const data = await client.get<GitLabProject>(`/projects/${encodeProjectPath(project)}`);
			return {
